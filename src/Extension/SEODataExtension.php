...
class SEODataExtension extends DataExtension
{
    ...
    public function MetaTagCollection($raw = false)
    {
        ...
        if ($record->obj('MetaKeywords')->getValue()) {
            $tags['keywords'] = $raw ? $record->obj('MetaKeywords') : HTML::createTag('meta', [
                'name' => 'keywords',
        ...
        if (empty($keyword)) {
            ...
            if ($duplicates->count()) {
                ...
                $result->addFieldError(
                    'FocusKeyword',
                    sprintf(_t(
                        __CLASS__ . '.FocusKeywordIsNotUnique',
                        'This keyword is not unique. It is also used by \'%s\''
                    ), $items),
                    ValidationResult::TYPE_ERROR,
                    null,
            ...
            if ($result->isValid()) {
                $result->addFieldMessage('FocusKeyword', _t(
                    ...
                    'This keyword is not used by any other pages on this site'
    ...
    public function validateMetaTitle(ValidationResult $result)
    {
        ...
        if (empty($metaTitle)) {
            $result->addFieldError(
                'MetaTitle',
                sprintf(_t(
                    __CLASS__ . '.MetaTitleEmpty',
                    'You have not set a meta title. The title will default to "%s"'
                ), $record->getTitle()),
            ...
            if ($record->FocusKeyword && strpos($metaTitle, $record->FocusKeyword) === false) {
                $result->addFieldError(
                    'MetaTitle',
                    sprintf(_t(
                        __CLASS__ . '.MetaTitleNoKeyword',
                        'The focus keyword "%s" does not appear in the SEO title.'
                    ), $record->FocusKeyword),
                    ValidationResult::TYPE_ERROR
                );
            }
            if (strlen($metaTitle) < 45) {
                ...
                $result->addFieldMessage(
                    'MetaTitle',
                    _t(__CLASS__ . '.MetaTitleLengthGood', 'The SEO title has a nice length.'),
                    ValidationResult::TYPE_GOOD
                );
            }
            ...
            if ($duplicates->count()) {
                ...
                $result->addFieldError(
                    'MetaTitle',
                    sprintf(_t(
                        __CLASS__ . '.MetaTitleDuplicated',
                        'This title is not unique. It is also used by %s'
                    ), $items),
                    ValidationResult::TYPE_ERROR,
                    null,
                    ValidationResult::CAST_HTML
    ...
    public function validateMetaDescription(ValidationResult $result)
    {
        ...
        if (empty($desc)) {
            $result->addFieldError(
                'MetaDescription',
                _t(
                    __CLASS__ . '.MetaDescriptionEmpty',
